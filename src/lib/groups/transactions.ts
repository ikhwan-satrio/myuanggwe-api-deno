...
import { transactionSchema } from "#server/lib/schemas.ts";
  ...
  .post("/create", zValidator("json", transactionSchema), async (c) => {
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        if (type === "transfer") {
          if (!toWalletId) {
            throw new Error("Destination wallet required for transfer");
          }
          await tx
  ...
  .put("/edit/:id", zValidator("json", transactionSchema), async (c) => {
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        if (!currentOldWalletSource) {
          throw new Error("Old source wallet not found or access denied");
        }
        ...
        if (oldType === "income") {
          ...
          const currentOldWalletDest = await tx.query.wallets.findFirst({
            where: and(
              eq(schema.wallets.id, oldToWalletId),
              walletContextQuery,
            ),
          });
          if (!currentOldWalletDest) {
            throw new Error(
              "Old destination wallet not found or access denied",
            );
          }
          await tx.update(schema.wallets)
            .set({ balance: currentOldWalletSource.balance + oldAmount })
          ...
          await tx.update(schema.wallets)
            ...
            .where(
              and(eq(schema.wallets.id, oldToWalletId), walletContextQuery),
            );
        }
        ...
        if (!newWalletSource) {
          throw new Error("New source wallet not found or access denied");
        }
        ...
        if (type === "transfer") {
          ...
          if (!newWalletDest) {
            throw new Error(
              "New destination wallet not found or access denied",
            );
          }
          await tx.update(schema.wallets)
