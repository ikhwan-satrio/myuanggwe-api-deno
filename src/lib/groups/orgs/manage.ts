...
export const manageOrgsGroup = new Hono()
  ...
  .delete('/members/:id', async (c) => {
    ...
    if (requester?.role !== 'owner') return c.json({ message: 'Forbidden' }, 403);
  ...
  .put('/members/:id/role', zValidator('json', z.object({ role: z.string() })), async (c) => {
    const user = c.get('user');
    const activeOrg = c.get('activeOrg');
    const { id } = c.req.param();
    ...
    await db.update(schema.member).set({ role }).where(eq(schema.member.id, id));
