import { organizationSchema, joinSchema, inviteSchema } from '#server/lib/schemas';
...
export const orgsGroups = new Hono()
  ...
  .post('/invite', zValidator('json', inviteSchema), async (c) => {
    ...
    if (!activeOrg) return c.json({ success: false, message: 'Harus memilih organisasi aktif' }, 400);

    try {
      await auth.api.createInvitation({
        body: { email: body.email, role: body.role, organizationId: activeOrg.id },
        headers: c.req.raw.headers as any
      });
  ...
  .post('/accept-invitation', zValidator('json', joinSchema), async (c) => {
    ...
    try {
      ...
      await invalidateUserCache(user.id);
      return c.json({ success: true, message: 'Berhasil bergabung dengan organisasi' });
    } catch (e) {
  ...
  .put('/change/:id', async (c) => {
    ...
    try {
      ...
      return c.json({
        success: true,
        message: targetOrgId ? 'Organization switched successfully!' : 'Switched to personal account!',
